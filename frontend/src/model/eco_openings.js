...
const ECO_OPENING_NAMES_BY_CODE = new Map((ecoOpeningsData?.openings || [])
  .map((entry) => {
    const code = normalizeEcoCode(entry?.code || "");
    if (!code) return null;
    const names = Object.fromEntries(Object.entries(entry?.names || {})
      .map(([lang, value]) => [normalizeLanguageCode(lang), String(value ?? "").trim()]));
    return [code, names];
  })
  .filter(Boolean));
...
export const resolveEcoOpeningName = (ecoCode, language = DEFAULT_LANGUAGE) => {
  ...
  const namesByLanguage = ECO_OPENING_NAMES_BY_CODE.get(normalizedCode);
