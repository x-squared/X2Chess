...
- `frontend/src/runtime/bootstrap_shared.ts` — `isLikelyPgnText`, `isLikelyFenText`, `isChess960StartingFen`, `fenToPgn`
...
- [x] **OPEN-3a** — Pasting the standard starting FEN opens a plain new game (no SetUp/FEN headers). Pasting a Chess960 starting FEN opens a game with `Variant "Chess960"`. Pasting any other FEN opens a custom-position game with `SetUp "1"` and `FEN` headers.
