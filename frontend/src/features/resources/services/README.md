...
- `game_rendering.ts`: game rendering profile (GRP) — `resolveDisplay`, `buildRenderedGameMap`; pass schema `fields` so select-type rule conditions match case-insensitively (PGN vs saved rule values)
