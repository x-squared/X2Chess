 ...
 *
 * @param {Chess} game - Source game instance to clone.
 * @returns {Chess} Independent game instance with identical position.
 */
 ...
 *
 * @param {Chess} game - Mutable chess game instance.
 * @param {string} san - SAN token to apply.
 * @returns {object|null} `chess.js` move object or null when all candidates fail.
 */
export const applySanWithFallback = (game, san) => {
 ...
 *
 * @param {object} pgnModel - Parsed PGN model.
 * @returns {Record<string, object>} Move position metadata by move id.
 */
export const buildMovePositionById = (pgnModel) => {
  const index = {};
 ...
 * @returns {object|null} Resolved position metadata or null when unresolved.
 */
export const resolveMovePositionById = (pgnModel, moveId) => {
  ...
  return walkVariation(pgnModel.root, new Chess(), true, 0);
};
 ...
 *
 * @param {object} pgnModel - Parsed PGN model.
 * @returns {Record<string, number>} Mainline ply by move id.
 */
export const buildMainlinePlyByMoveId = (pgnModel) => {
  const byId = {};
 ...
 *
 * @param {string} source - Raw PGN source.
 * @returns {string} PGN with only mainline tokens preserved.
 */
export const stripAnnotationsForBoardParser = (source) => {
  let out = "";
