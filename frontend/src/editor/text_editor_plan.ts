...
const addComment = (state, comment) => {
  ...
  const isPlain = layoutMode === "plain";
  const isStructured = layoutMode === "text" || layoutMode === "tree";
  ...
  const hadIntroDirective = hasIntroDirective(comment);
  const withoutIntro = hadIntroDirective ? stripIntroDirective(rawText) : rawText;
  ...
  const introStyling = isStructured && isFirstComment;
  const focusFirstCommentAtStart = isStructured && isFirstComment;
  addCommentToken(
    state,
    comment,
    visibleText,
