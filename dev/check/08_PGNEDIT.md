...
- [ ] **PGNEDIT-89** — Loading PGN text that contains empty comments (`{}`, `{[[br]]}`, `{<br/>}`, or only newline markers) drops those empty comment nodes immediately so they do not appear in the editor or in re-serialized PGN.
- [ ] **PGNEDIT-90** — Entering a black-side variation from the board (e.g. `5... Bd6`, then `Bg5`, `Ne7`) emits canonical variation numbering with a white fullmove token (`5... Bd6 6. Bg5 Ne7`), not an unnumbered continuation.
